...
pub struct SqliteFootprints;

impl SqliteFootprints {
    pub fn find(conn: &SqliteConnection, id: i32) -> Result<Option<Footprint>, Box<dyn Error>> {
        use crate::db::schema::footprints::dsl;
        let q = dsl::footprints.find(id);
        Ok(q.first::<Footprint>(conn).optional()?)
    }

    pub fn select(conn: &SqliteConnection, ids: &Vec<i32>) -> Result<Vec<Footprint>, Box<dyn Error>> {
        use crate::db::schema::footprints::dsl;
        let q = dsl::footprints.filter(dsl::id.eq_any(ids));
        Ok(q.load::<Footprint>(conn)?)
    }

    pub fn find_by_digest(conn: &SqliteConnection, digest: &str) -> Result<Option<Footprint>, Box<dyn Error>> {
        use crate::db::schema::footprints::dsl;
        let q = dsl::footprints.filter(dsl::digest.eq(digest));
        Ok(q.first::<Footprint>(conn).optional()?)
    }

    pub fn insert(conn: &SqliteConnection, footprint_form: &FootprintInsertForm) -> Result<(), Box<dyn Error>> {
        use crate::db::schema::footprints::dsl;
...
impl SqliteHistories {
    ...
    pub fn select_by_path(conn: &SqliteConnection, group_id: &str, path: &str) -> Result<Vec<History>, Box<dyn Error>> {
        use crate::db::schema::histories::dsl;
...
pub struct SqliteGroups;

impl SqliteGroups {
    pub fn find(conn: &SqliteConnection, id: &str) -> Result<Option<Group>, Box<dyn Error>> {
        use crate::db::schema::groups::dsl;
        let q = dsl::groups.find(id);
        Ok(q.first::<Group>(conn).optional()?)
    }

    pub fn select(conn: &SqliteConnection, ids: &Vec<&str>) -> Result<Vec<Group>, Box<dyn Error>> {
        use crate::db::schema::groups::dsl;
        let q = dsl::groups.filter(dsl::id.eq_any(ids));
        Ok(q.load::<Group>(conn)?)
    }

    pub fn select_all(conn: &SqliteConnection) -> Result<Vec<Group>, Box<dyn Error>> {
        use crate::db::schema::groups::dsl;
        let q = dsl::groups;
        Ok(q.load::<Group>(conn)?)
    }

    pub fn insert(conn: &SqliteConnection, group_form: &GroupInsertForm) -> Result<(), Box<dyn Error>> {
        use crate::db::schema::groups::dsl;
    ...
    pub fn insert_and_find(conn: &SqliteConnection, group_form: &GroupInsertForm) -> Result<Group, Box<dyn Error>> {
        Self::insert(conn, group_form)?;
        let group = Self::find(conn, group_form.id)?;
        Ok(group.unwrap())
    }

    pub fn update(conn: &SqliteConnection, id: &str, group_form: &GroupUpdateForm) -> Result<(), Box<dyn Error>> {
        use crate::db::schema::groups::dsl;
...
impl SqliteStats {
    ...
    pub fn find_by_path(conn: &SqliteConnection, group_id: &str, path: &str) -> Result<Option<Stat>, Box<dyn Error>> {
        use crate::db::schema::stats::dsl;
        let q = dsl::stats.filter(dsl::group_id.eq(group_id)).filter(dsl::path.eq(path));
        Ok(q.first::<Stat>(conn).optional()?)
    }

    pub fn select_by_group_id(conn: &SqliteConnection, group_id: &str) -> Result<Vec<Stat>, Box<dyn Error>> {
        use crate::db::schema::stats::dsl;
